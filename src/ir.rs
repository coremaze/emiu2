...
pub trait IrInterface {
    ...
    /// The oscillator cycle of the next already-scheduled received edge,
    /// `u64::MAX` if none is scheduled. Edges still in flight from the
    /// network are not visible here; they are discovered by a regular
    /// poll and scheduled `REPLAY_LATENCY_NS` out, which is far larger
    /// than the poll interval.
    fn next_edge_cycle(&self) -> u64 {
        u64::MAX
    }
}
