...
Players should start here: run `emiu2-desktop` (or `cargo run -r -p emiu2-desktop`), pick a character, and play. No image files or flags are needed — every character's 1.09.03 firmware is built in (custom OTP/flash images remain available under the new-save dialog's advanced options).
...
- **Friends** wraps the IR relay: it shows your friend code, joins a friend's, and pairs — then take both Miuchiz to their wireless feature, like holding two real ones face to face.
