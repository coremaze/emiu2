...
impl GpioInterfaceInternal for IrCircuit {
    ...
    fn next_event_cycle(&self) -> u64 {
        // A scheduled edge is observed at the first receive poll at or
        // after it, so an edge inside the poll rate limit is due exactly
        // when the limit expires.
        let edge = self.transceiver.next_edge_cycle();
        let mine = if edge == u64::MAX {
            u64::MAX
        } else {
            edge.max(self.next_rx_poll)
        };
        mine.min(self.io.next_event_cycle())
    }
